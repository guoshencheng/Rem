...
注意：本规范约束的是"拆分"的手段与红线，不是要求见逻辑就拆。只有当前一个场景使用的逻辑应保持内聚，参见核心原则第 1 条。

## 核心原则

### 1. 内聚优先于形式拆分

只有当前一个场景使用的逻辑，应内聚在调用它的文件/模块里，不要为了形式上的"单一职责"拆成独立元素或文件。即使这意味着两个文件之间存在少量重复（如 ~10 行的闭包构造、事件发布模板），保持重复也是允许的、甚至是推荐的。

单文件自洽、读完不用跳文件、依赖一眼可溯，比形式上的 DRY 更有价值。一段重复逻辑只有满足以下任一条件时才抽成共享元素：

- 出现第三个使用点
- 该逻辑开始独立演化（规则变复杂、需要独立测试）

第二次出现时保持重复。拆分是手段而非目的：拆分的正当理由是多个真实场景已经存在，或存在统一分发/替换点需要稳定接口（如策略、插件），而不是"一个文件做了两件事就该拆"。

### 2. 单一职责

每个文件只负责一件事（在不违反上一条内聚原则的前提下）：
...
超过建议行数时，主动考虑拆分；超过绝对上限时，必须拆分。拆分前先确认拆出的部分有多个真实使用场景、独立演化需求，或属于 Provider/策略这类需要稳定接口的替换点；仅为压缩行数而把单场景逻辑挪走不算正当拆分。
...
1. [ ] 这段逻辑是否只有一个使用场景？是则不要拆，保持内聚在调用方文件里
2. [ ] 这段重复逻辑是否已出现第三个使用点，或开始独立演化？否则保持重复
3. [ ] 这个文件是否只承担一种职责？
4. [ ] 文件行数是否在建议范围内？
5. [ ] 类型定义是否抽离到了 `types.ts`？
6. [ ] 抽象接口是否抽离到了 `sdk/`？
7. [ ] 具体实现是否放入 `plugins/` 或 `defaults/`？
8. [ ] 是否通过 `index.ts` 聚合导出？
9. [ ] 新文件的导入是否遵循 type/value 分离、`.js` 扩展名？
...
- 把只在单一场景使用的逻辑拆成独立文件，导致读代码要跳多个文件、依赖链不可溯（过度拆分）
