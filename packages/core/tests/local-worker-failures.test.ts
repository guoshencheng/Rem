...
describe('LocalRunWorker 失败完成', () => {
  ...
  it('Artifact 插入失败时成功事务整体回滚，再以 INTERNAL_ERROR 原子失败', async () => {
    const base = await fakeStore(); await seedRun(base);
    const storage = failArtifactWrites(base);
    await createWorker(storage, { execute: async () => successResult }).drainOne();
    ...
    expect(await base.getRun('run-1')).toMatchObject({ status: 'failed', errorCode: 'INTERNAL_ERROR' });
