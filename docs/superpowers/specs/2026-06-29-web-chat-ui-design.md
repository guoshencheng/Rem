...
视觉方向：**现代深色**，主点缀色 **薰衣草紫 `#b9a9ff`**。仅深色主题，不做浅色。

可视化设计稿（HTML mockup）存档于 `.superpowers/brainstorm/`：
- `visual-direction.html` — 三方向对比（已选深色）
- `dark-lavender.html` — 主屏完整态
- `design-system.html` — 色板 + 空状态 + 移动端
- `components-states.html` — 组件全状态总览（视觉定稿）
...
| `--bg` | `#0f1318` | 主背景 |
| `--sb` | `#151a21` | 侧边栏背景 |
| `--card` | `#1a2129` | 卡片 / 气泡 / 输入框背景 |
| `--card2` | `#161c23` | 次级卡片（推理块/工具块展开体） |
| `--bd` | `#232b35` | 主边框 |
| `--bd2` | `#2a333e` | 输入/控件边框 |
| `--tx` | `#e6ebf1` | 主文字 |
| `--tx2` | `#9aa7b4` | 次要文字 |
| `--tx3` | `#5d6b7a` | 占位 / 弱文字 |
| `--ac` | `#b9a9ff` | 主点缀（按钮、用户气泡、高亮、链接、代码关键字） |
| `--ac-ink` | `#1c1635` | 紫底之上的深色文字 |
| `--ac-soft` | `#2a2350` | 紫色弱底（推理块头部、空状态图标底） |
| `--ok` / `--ok-bg` | `#6ee7b7` / `#10241d` | 成功（工具完成、字符串高亮） |
| `--err` / `--err-bg` | `#fb7185` / `#2a1620` | 错误（消息错误、工具失败、删除） |
| `--warn` / `--warn-bg` | `#fbbf24` / `#2a2310` | 警告（重连提示） |

### 基础规范

- **字体**：界面用系统无衬线；代码块、工具名用等宽（SF Mono / monospace）。
- **圆角**：卡片/气泡 14px，按钮 9px，chip 18px。
- **间距**：4px 栅格基准。
- **聚焦态**：输入框聚焦时 `--ac` 描边 + `0 0 0 3px rgba(185,169,255,.12)` 外发光。
- **当前会话**：`--card` 背景 + 左侧 2px `--ac` 边框。

### 组件状态清单（设计稿已覆盖）

| 组件 | 状态 |
|------|------|
| Message | 用户气泡（紫底深字）/ streaming（仅 ThinkingBar）/ done / error（红框 + 重试） |
| ThinkingBar | 转圈 + "Thinking" + 三点跳动动画 |
| ReasoningBlock | 折叠（`▶ ✦ Thinking`）/ 展开（紫色头部 + 转圈 + 斜体思考文本） |
| ToolCallBlock | executing（转圈灰）/ done 折叠（绿勾 + 摘要）/ done 展开（入参/出参）/ error（红叉 + 错误信息） |
| Markdown | 粗斜体、行内代码、有序/无序列表、引用、表格、代码块（高亮 + 横向滚动 + max-height 140px） |
| InputBox | 默认/聚焦 / 流式中（禁用 + "中断"按钮）/ 服务异常禁用 |
| SessionItem | 普通 / 选中 / 置顶 / 重命名编辑态 / 右键菜单 / 删除确认对话框 |
| 全局反馈 | SSE 重连提示条（黄）/ 服务异常 banner（红）/ Toast（超时、中断失败） |
| 空状态 | 空消息列表（欢迎 + 快捷 chip）/ 会话列表为空 / 搜索无结果 |

### 响应式

- **≥768px**：左侧边栏常驻 + 右聊天区双栏布局。
- **<768px**：聊天区全屏，侧边栏收为抽屉；点 Header 的 `☰` 从左推入抽屉，带半透明遮罩，点遮罩关闭。

---

**视觉设计稿状态：已定稿**（深色 + `#b9a9ff`，见 `.superpowers/brainstorm/` 存档）。
