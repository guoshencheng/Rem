...
interface ClaimWorkerData {
  dbPath: string; owner: string; gate: SharedArrayBuffer; mode: 'claim' | 'exit-before-result';
}

const { dbPath, owner, gate, mode } = workerData as ClaimWorkerData;
...
try {
  ...
  if (mode === 'exit-before-result') { db.close(); db = undefined; process.exit(0); }
